    ...
    TimeProvider timeProvider)
{
        ...
        Guid periodId, Guid actorUserId, CancellationToken cancellationToken = default)
    {
        ...
        var pdf = CreatePdf(tenantName, report, generatedAt);
    ...
    private static byte[] CreatePdf(string tenantName, DreReport report, DateTimeOffset generatedAt)
    {
        ...
        return Document.Create(document => document.Page(page =>
        {
            ...
            page.Header().Column(column =>
            {
                ...
                column.Item().Text(tenantName).FontSize(18).Bold();
                column.Item().Text($"Competencia {report.Month:D2}/{report.Year} | Revisao {report.Revision} | Gerado em {generatedAt:dd/MM/yyyy HH:mm} UTC")
                    .FontColor("#64748B");
            });
            page.Content().PaddingTop(24).Column(column =>
            {
                ...
                Row("Receita bruta", report.Totals.GrossRevenue, true);
                Row("(-) Deducoes", -report.Totals.Deductions);
                Row("(-) Tributos sobre vendas", -report.Totals.Taxes);
                Row("Receita liquida", report.Totals.NetRevenue, true);
                Row("(-) Custo das mercadorias vendidas", -report.Totals.Cmv);
                Row("Lucro bruto", report.Totals.GrossProfit, true);
                Row("(-) Despesas de vendas", -report.Totals.SellingExpense);
                Row("(-) Despesas operacionais", -report.Totals.OperatingExpense);
                Row("RESULTADO DO PERIODO", report.Totals.Result, true);
