    ...
    TimeProvider timeProvider)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var authentication = await connector.RefreshTokenAsync(
                new RefreshTokenRequest(RequireTenant(), Protector(connection).Unprotect(connection.EncryptedRefreshToken)), cancellationToken);
            ValidateAuthentication(authentication);
            ApplyAuthentication(connection, authentication);
            await db.SaveChangesAsync(cancellationToken);
        }
    ...
    private IDataProtector Protector(MarketplaceConnection connection) =>
        _dataProtectionProvider.CreateProtector(
            "BraSeller.ConnectorTokens.v1", connection.TenantId.ToString(), connection.ConnectorName);
}
