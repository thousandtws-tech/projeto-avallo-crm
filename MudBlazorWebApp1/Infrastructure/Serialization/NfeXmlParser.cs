...
public sealed class NfeXmlParser : INfeXmlParser
{
    public ParsedNfeInvoice Parse(Stream xml)
    {
        ...
        try
        {
            ...
            if (parsedItems.GroupBy(x => x.SupplierSku, StringComparer.Ordinal).Any(x => x.Count() > 1))
                throw Invalid("NF-e contains duplicate supplier item codes.");
            ...
            return new ParsedNfeInvoice(
                ...
                issuedAt,
                RequiredSingleChildValue(emit, "CNPJ"),
